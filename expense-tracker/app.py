...
from database.db import get_db, init_db, seed_db, create_user, get_user_by_email, get_user_by_id, get_profile_stats, get_recent_expenses, get_category_breakdown, insert_expense, delete_expense, get_monthly_totals, get_analytics_stats, get_expense_by_id, update_expense
...
def analytics():
    import json
    user_id       = g.user['id']
    stats         = get_analytics_stats(user_id)
    categories    = get_category_breakdown(user_id)
    monthly       = get_monthly_totals(user_id, months=6)

    monthly_labels = json.dumps([m['month'] for m in monthly])
    monthly_data   = json.dumps([m['total'] for m in monthly])
    cat_labels     = json.dumps([c['name']  for c in categories])
    cat_data       = json.dumps([c['total'] for c in categories])
...
def add_expense():
    ...
    insert_expense(g.user['id'], amount, category, expense_date, description)
...
def edit_expense(id):
    ...
    try:
        ...
        return render_template("edit_expense.html",
                               error="Amount must be a number greater than 0.",
                               expense=expense, amount=amount_raw,
    ...
    update_expense(id, g.user['id'], amount, category, expense_date, description)
...
def delete_expense_route(id):
    deleted = delete_expense(id, g.user['id'])
    if deleted:
